...
#### 개념
- 하드웨어 차원에서 구현된 원자적 연산으로, CAS(Compare-and-Swap) 연산은 락 없이도 일정 부분 원자성을 보장할 수 있음
- CAS 연산의 원자성은 CPU 명령어 수준에서 보장되며, 메모리 특정 위치의 값이 예상한 값과 일치하면 새로운 값으로 교체하고, 일치하지 않으면 값을 변경하지 않고 종료됨
- CAS 연산을 이용해 락(스핀락)을 구현할 수 있으며, 스핀락은 플래그 변수를 통해 락의 상태를 관리하고, 스레드는 락을 획득하기 위해 CAS 연산을 반복 수행하며 busy waiting 상태로 대기함

#### 성능
- CAS 연산은 원자적으로 수행되지만, 이를 사용하는 코드에서 내부적으로 재시도하는 로직이 포함될 수 있어 busy waiting이 발생할 수 있으며, 이로 인해 CPU 자원을 비효율적으로 소모함
- 따라서 충돌이 잦은 환경에서는 busy waiting으로 인해 CAS 연산이 일반적인 동기화 락보다 성능이 떨어질 수 있음
- 복잡한 동기화 작업에는 동기화 락을 사용하는 것이 좋으며, 간단한 연산이나 충돌이 적은 환경에서만 CAS 연산을 사용하는 것이 권장됨
<details>  
