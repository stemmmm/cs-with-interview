...
- CAS 연산을 이용해 락(스핀락)을 구현할 수 있으며, 메모리의 플래그 변수를 통해 락의 상태를 관리하고, 스레드는 락을 획득할 때까지 CAS 연산을 반복 수행하며 busy waiting 상태로 대기함

#### 성눙
- 충돌이 잦은 환경에서는 busy waiting으로 인해 오히려 락보다 성능이 떨어질 수 있음
- CAS 연산을 사용하는 코드는 `while` 문을 통해 반복적으로 재시도하는 로직이 많으므로 busy waiting이 발생해 CPU 자원을 비효율적으로 소모하게 됨
- 따라서 복잡한 동기화 작업에는 동기화 락을 사용하고, 간단한 연산이나 충돌이 적은 환경에서만 CAS 연산을 사용하는 것이 권장됨
<details>  
